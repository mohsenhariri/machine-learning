...
from data.mnist_resized import train_loader, test_loader
...
while True:
    j += 1
    file_exists = path.exists(f"./mnist/runs/alex-{j}")
    if not file_exists:
        break

writer = SummaryWriter(log_dir=f"./mnist/runs/alex-{j}")


model = AlexNet(num_classes=10, is_colored=False, input_size=(224, 224))
